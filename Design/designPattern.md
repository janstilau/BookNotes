...
如有一个类的内部, 有一个状态值, 然后它的很多操作, 都要通过这个state值进行判断, 比如说, 一个大的switch语句, 然后这个switch语句中, 包含了复杂的处理逻辑, 那么就可以考虑用state模式, 如果有很多的行为, 都要进行这样的switch判断的话, 那么用state模式就显得很有必要了.

首先, switch的语句会很长, 里面会有很多的逻辑判断, 将所有的判断聚集在一个方法里面, 逻辑混乱, 而且稍微有点改动的时候, 其实会影响整个代码段, 太过于危险. 运用state模式, 可以让每个不同的state对象, 处理处于自己state下的情况, 修改的危险会小很多, 而且逻辑也会显得清晰.

在逻辑上, 包含state实体的上下文, 将自己的操作, 完全代理给state对象, 上下文也可以将自己传入给state对象, 这样可以方便state对象在需要模式改变的时候, 改变context的模式. 这样, 就可以在state的内部, 进行模式切换的管理. 比如, 运行态可以在运行完成之后, 生成一个结束态对象, 然后将这个结束态对象, 设置给conetxt的state成员变量, 这样就实现了state内部管理切换的过程.

### 责任链模式

这个模式其实用的很多, 最主要的应用还是在事件的处理上面. 在鼠标, 键盘, 还是手机屏幕被触摸之后, 以iOS为例, 其实会有一个touchesBegin的调用, 如果当前类没有重写这个方法, 这个方法默认会调用它的下一个相应者的同样方法. 所以, 如果想要构造这样的一个结构也是很简单的, 定义一个类, 类里面有一个方法, 也会有一个这个类的成员变量来保存它的下一个响应者的应用, 然后这个方法有一个默认实现, 默认实现, 就是调用下一个响应者的这个方法.

那么想象一下这个模式的用途, 用现有的框架解释一下. 在一个窗口的内部, 进行右键点击, 这个时候应该调出一个上下文菜单, 这个处理应该是在mousePress事件的处理函数里面, 在这个处理函数里面, 我们会判断一下是不是鼠标右键单击的, 如果是我们就调出自己配置好的专门用于这个窗口内部的上下文菜单, 如果不是的话, 就调用super 的mousePress事件. 那么, 如果我们在这个特定窗口的下一个响应者, 比如父窗口里面也重载了这个函数呢. 比如我们重载成为, 按住shift的鼠标左键单击, 这个模式在原来那个特定窗口不会被识别出来, 所以来到了它的父窗口, 而这个父窗口对这个模式进行了识别. 而这个父窗口中, 对于其他的没有识别的模式, 会继续调用它的下一个响应者的mousePress的处理函数.

所以, 在鼠标按下的时候, 我们是不会知道, 到底是哪一个对象真正的处理这个事件的. 事件的处理函数, 会在自己不能处理之后, 将这个事件上传给自己的下一个响应者, 这个下一个响应者, 一般会利用当前已有的结构, 比如, 它的父窗口对象, 在iOS里面, view是这样处理的, 在view是vc的view的时候, 这个view的下一个响应者就是vc了, vc的下一个响应者是这个vc的父vc. 这样, 其实到底某个事件被谁处理了, 很难决定. 这样设计有什么好处呢, 最大的好处就是, 我们可以让一个顶层的对象, 处理大部分的事件, 然后让一个特定的对象, 处理某个特定的事件, 这样的话, 我们编码的时候, 其实有了很强的便利性, 只用专门的截获特定的事件就可以了, 其他原有的事件处理, 调用系统默认的行为就可以了. 
...
* 适配器模式的使用场景是什么呢, 不应该是设计开始的时候, 而是在有动机地修改一个正常运行的系统的接口, 如果过多的使用适配器模式, 会让系统凌乱, 明明调用的一个接口, 内部实际上是对于另外一个接口的调用, 所以, 如果有必要, 应该对于代码进行重构处理.

* 简单的模型是: 原有一个类实现了具体的功能, 但是和现需要的接口不一致, 那么, 新造一个适配器类, 这个类的内部, 包含一个具体的实现类的对象, 然后, 适配器去实现所有的现需要的接口, 但是在接口的具体实现内部, 调用成员变量的方法.

* 这个模式, 在动态语言里面, 几乎用不到, 比方说, 在oc里面可以任意给类加分类, 也就是可以在不改变原有的实现文件的基础上, 给类添加功能. 所以, 接口不一致的问题, 很好的就解决了. 不过, 给类加分类的方法, 其实会改变整个类的实现, 具体用什么策略, 还是要看当时的选择.

* 和代理模式的区别: 代理模式是不会改变实现类的接口的, 代理模式主要是为了增加对于实现类的控制, 而适配器模式则不会改变实现, 主要是为了改变接口.
...
1. remote proxy, 在proxy里面, 其实要管理着链接网络等等操作.
1. virtual proxy, create expensive objects on demand, 例如书中有一个imageProxy, 只有在image真正需要被展示的时候, 才会去取照片的数据, 其他的情况, 当不用真正生成image的时候, 尽量不去做昂贵操作. 例如, 在内部存储了image的size信息, 在排版类需要的时候直接返回, 满足排版类的需要, 但是只有在需要显示的时候, 才会真正去生成一个image.
1. protection proxy. 里面应该有着权限的控制.

书中例子,用代理模式实现copy-on-write, 在代理里面, 除了保持一个指针之外, 还保持一个公共的引用计数, 当copy的时候, 仅仅是把这个指针指向添加到新的代理, 然后这个公共的引用计数加一. 当真正改变这个引用的对象的值的时候, 再去新造一个对象, 然后更改引用和引用计数. 这一些的操作, 都是在代理类中完成.

注意, 代理类和被代理的类, 必须是同一个type, 就是说, 代理类要完全实现被代理类的所有的公共接口.

如果代理类代理的就是一个抽象基类, 那么只用实现这个抽象基类的接口就可以了. 具体的实现细节是抽象基类的子类完成的. 代理类只用管理自己的那一部分功能, 例如defer某些操作, 或者权限保护等.
